...
RE: RE '|' RE {
  int s1, s2;
  s1 = create_state(); s2 = create_state();
  create_transition(s1, FIRST($1), EPSILON); create_transition(LAST($1), s2, EPSILON);
  create_transition(s1, FIRST($3), EPSILON); create_transition(LAST($3), s2, EPSILON);
  $$ = createRE(s1, s2);
}
;

/* Kleene's star (transitive closure) */
RE: RE '*' {
  int s1, s2;
  s1 = create_state(); s2 = create_state();
